...
If `startRTMS` fails:
- Confirm your app runs inside Zoom, not a normal browser.
- Confirm the in-app log shows `Zoom Apps SDK script loaded` and `Zoom Apps SDK configured`.
- Confirm the in-app log shows `Supported Zoom App APIs` with `startRTMS: true` and `stopRTMS: true`.
- Confirm your Zoom App capabilities include `startRTMS` and `stopRTMS`.
- Confirm the app has the in-meeting Zoom App scope.
- Confirm `https://appssdk.zoom.us/` is in the Zoom App allow list.
- If `https://appssdk.zoom.us/sdk.js` is `403 Blocked`, this is not an RTMS race condition. The Zoom client is blocking the SDK script before your app can initialize it.
- If you see `must call zoomSdk.config before using other API methods` during SDK bootstrap, make sure your loader is not resolving a Promise with `window.zoomSdk`. Wait for the SDK, then read it, then call `zoomSdk.config(...)`.

If no webhook arrives:
- Confirm the Marketplace webhook URL points to `/webhook`.
- Confirm `meeting.rtms_started` and `meeting.rtms_stopped` are subscribed.
- Confirm your public URL is reachable over HTTPS.

If the participant dropdown is empty:
- Confirm the meeting has participants with camera on.
- Confirm `VIDEO_STREAM_MODE=individual`.
- Confirm the RTMS library receives `PARTICIPANT_VIDEO_ON` / `PARTICIPANT_VIDEO_OFF`.

If video preview does not play:
- Confirm `ffmpeg -version` works.
- Confirm `npm install` completed successfully so `/vendor/hls.min.js` is actually available.
- Check the server logs for `[ffmpeg:hls]` messages.
- Open `/hls/stream.m3u8` after media starts to confirm HLS output exists.
- The sample now starts ffmpeg with `FFMPEG_THREAD_QUEUE_SIZE=1024` by default for both HLS and clip-generation inputs. If you still see `Thread message queue blocking`, raise it to `2048`.
- A non-fatal `HLS warning: mediaError / bufferStalledError` during startup or right after switching streams is usually a transient wait for the next segment, not a broken pipeline.

If local inference is slow:
- Prefer `DEEPFAKE_MODE=service` for the normal app path.
- For same-machine debugging, point `DEEPFAKE_SERVICE_URL` at `http://127.0.0.1:8012`.
- Lower `DEEPFAKE_FRAME_FPS` or shorten `DEEPFAKE_CLIP_SECONDS` if CPU usage is too high.

If scores suddenly show `Real score 0.000` and `Fake score 1.000`:
- Check the Python service log for `classification_result`.
- Compare the frontend `Clip` value with the log `clip` / `metadata.clipPath` value.
- Check `videoInfo.frameCount`, `videoInfo.durationSeconds`, and `tensorStats.std`; empty, short, or near-constant clips can produce saturated model output.
- Check `logits`; if the fake logit is much larger than the real logit, the model itself is making a high-confidence fake prediction.

If the deepfake service health check fails:
- Confirm the standalone service is running.
- Confirm `curl http://127.0.0.1:8012/video/health` returns `ok: true`.
- Confirm `DEEPFAKE_SERVICE_URL` matches the service host/port or remote domain.
