...
export function generateSignature(sessionID: string, streamId: string, clientId: string, clientSecret: string): string {
    const message = `${clientId},${sessionID},${streamId}`;
    return crypto.createHmac('sha256', clientSecret).update(message).digest('hex');
}

export function logRtmsSessionState(stateCode: number): void {
    switch (stateCode) {
        case 0:
...
export function logRtmsStreamState(stateCode: number): void {
    switch (stateCode) {
        case 0:
...
export function logRtmsStopReason(errorCode: number): void {
    switch (errorCode) {
        case 0:
...
export function logRtmsStatusCode(statusCode: number): void {
    switch (statusCode) {
        case 0:
