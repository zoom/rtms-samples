...
This example receives audio, video, and transcript data from a Zoom Video SDK session through RTMS. It routes incoming media by `msg_type` and logs connection and media metadata rather than writing media files.
...
1. Start the server:
```bash
gunicorn --bind 0.0.0.0:3000 index:app
...
4. Start a Video SDK session and begin streaming. The server will classify incoming audio, video, and transcript messages and log their metadata.
