# Zoom Video SDK RTMS Media Receiver (Spring Boot)

This Spring Boot example demonstrates how to receive real-time audio, video, screen share, transcript, and chat data from a Zoom Video SDK session using RTMS (Real-Time Media Streams).
...
- Zoom Video SDK app credentials (Client ID and Client Secret)
...
3. Set your Zoom Video SDK app's Event Notification URL to point to your ngrok endpoint, e.g.:
```
https://<your-ngrok-subdomain>.ngrok.io/webhook
```

4. Start a Zoom Video SDK session and initiate RTMS streaming.
...
└── service/
    ├── RtmsService.java                     # Core RTMS service with WebSocket clients
...
```
Video SDK webhook ──► WebhookController ──► RtmsService
       │                       │                    │
       └── Validation Response ◄─┘                    │
                                                    │
                                                    ▼
