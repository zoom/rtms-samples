...
let lastFragmentTimecodeMs = 0; // Track across sessions
...
function buildFfmpegArgs(offsetSeconds) {
  return [
    ...
    // Map inputs: video = #1, audio = #0
    '-map', '1:v:0', '-map', '0:a:0',
    '-vsync', 'cfr', '-r', '25',

    // Re-encode video to enforce keyframes + headers
    '-c:v', 'libx264',
    '-preset', 'veryfast',
    '-tune', 'zerolatency',
    '-profile:v', 'baseline',
    '-level', '4.0',
    '-x264-params', 'bframes=0:keyint=25:min-keyint=25:scenecut=0:repeat-headers=1:aud=1',
    '-g', '25', '-keyint_min', '25',
    '-pix_fmt', 'yuv420p',

    // Encode audio AAC 16kHz
    '-c:a', 'aac', '-b:a', '64k', '-ar', '16000', '-ac', '1',

    // Fragment muxing for KVS
    '-max_interleave_delta', '0',
    '-f', 'matroska',
    '-cluster_time_limit', '1000',       // ~1s fragments
    '-cluster_size_limit', '2097152',    // ~2MB max cluster
    'pipe:1'
  ];
}
...
function startFFmpeg() {
  ...
  const offsetSeconds = (lastFragmentTimecodeMs + 100) / 1000; // +100ms safety
...
function stopStream() {
  audioIn.end(); videoIn.end();
  if (ffmpegProcess && !ffmpegProcess.killed) ffmpegProcess.kill('SIGTERM');
