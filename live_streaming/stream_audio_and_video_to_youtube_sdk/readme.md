# Zoom RTMS Video and Audio YouTube Streaming project

This project demonstrates real-time audio and video capture using the Zoom RTMS. It streams the Audio and Video to YouTube via ffmpeg in real-time.
...
- `YOUTUBE_STREAM_KEY`: Streaming key to stream for your YouTube channel
...
4. When a meeting starts:
   ...
   - Setup pipes (video and audio) for YouTube streaming
...
5. During the meeting:  
   ...
   - ffmpeg reads the pipes and stream them to YouTube
   - Handles any connection errors
6. When a meeting ends:  
   - Receives `meeting.rtms_stopped` event
   - Stops ffmpeg from streaming to YouTube any further
...
2. Start a Zoom meeting. The application will:  
   ...
   - Setup audio and video pipe to prepare for YouTube streaming via ffmpeg
   - Begin receiving audio and video data, send them as buffer into respective audio and video pipes
   - After meeting ends, stops ffmpeg streaming to YouTube
...
- FFmpeg streams to YouTube by retrieve audio and video buffers from pipes
...
- Audio and Video buffer is first passed into pipes. FFmpeg retrieves these Audio/Video buffer from the pipes and streams them to YouTube
...
1. **YouTube Stream does not work Generated**:
