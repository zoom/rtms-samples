...
console.log('[AWS Transcribe] App Configuration:', appConfig);
...
if (appConfig.managerType === 'webhook') {
  ...
  console.log('[AWS Transcribe] Webhook Manager initialized');
  ...
  websocketManager.on('event', (event, payload) => {
    console.log('[AWS Transcribe] Websocket Event:', event);
    RTMSManager.handleEvent(event, payload);
  });

  await websocketManager.start();
  console.log('[AWS Transcribe] Websocket Manager initialized');
}
...
RTMSManager.on('error', (error) => {
  console.error('[AWS Transcribe] RTMS Error:', error.message);
});
...
process.on('uncaughtException', (err) => {
  if (err.code === 'ERR_STREAM_PREMATURE_CLOSE') {
    console.error('[AWS Transcribe] Stream closed prematurely (handled)');
    return;
  }
  console.error('[AWS Transcribe] Uncaught exception:', err);
  process.exit(1);
});
