...
const rtmsConfig = {
  ...
  mediaParams: {
    audio: {
      // RTMS sends one 16 kHz mono PCM16 stream per participant. Individual
      // packets retain their userId so they can be routed to separate sessions.
      ...
      dataOpt: MEDIA_PARAMS.MEDIA_DATA_OPTION_AUDIO_MULTI_STREAMS,
      sendRate: 100
    }
  }
};
...
app.get('/health', (req, res) => {
  res.json({
    ...
    scribePool: activeMeetingId ? getPoolSnapshot(activeMeetingId) : null
  });
});
...
// Forward each participant's RTMS audio packet to the Live Scribe layer.
RTMSManager.on('audio', ({ buffer, userId, userName, timestamp }) => {
  if (!activeMeetingId) return;
  if (userId != null && userName) participantNames.set(String(userId), userName);
  sendAudioChunk(buffer, activeMeetingId, {
    userId: userId ?? 'unknown',
    userName: userName || participantNames.get(String(userId)) || 'Unknown participant',
    timestamp
  });
});

RTMSManager.on('event', ({ eventType, data }) => {
  if (eventType !== 3 || !data) return; // PARTICIPANT_JOIN
  const participants = Array.isArray(data.participants) ? data.participants : [data];
  for (const participant of participants) {
    if (participant?.user_id != null && participant.user_name) {
      participantNames.set(String(participant.user_id), participant.user_name);
    }
  }
});
